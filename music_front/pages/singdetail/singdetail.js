...
Page({
  ...
  async onLoad(options) {
    ...
    if (!songid) { //点击全部播放进入
      // console.log(JSON.parse(decodeURIComponent(options.songs)));
      this.setData({
        songid: JSON.parse(decodeURIComponent(options.songs))[0].id,
        songs: JSON.parse(decodeURIComponent(options.songs)),
      })
    } else { //点击单曲进入
      this.setData({
        songid,
  ...
  async switchsong(e) {
    ...
    if (switchway === 'pre') { //切上一曲时
      if (this.data.cursongindex === 0) { //若当前为第一首
        this.setData({
          cursongindex: this.data.songs.length - 1,
        })
      } else {
        this.setData({
          cursongindex: this.data.cursongindex - 1,
        })
      }
    }
    if (switchway === 'next') { //切下一曲
      if (this.data.cursongindex === this.data.songs.length - 1) { //若当前为第一首
        this.setData({
          cursongindex: 0,
        })
      } else {
    ...
    this.setData({
      songid: this.data.songs[this.data.cursongindex].id,
      isPlay: true,
      scrollTop: 0,
      lrclocation: 0,
    })
  ...
  async getsongInfo() {
    ...
    this.findsongindex(); //获取当前音乐在歌单索引
    //判断当前是否为收藏歌
    this.judgeCollect();
    //添加历史记录
    this.addhistory();
  ...
  matchLrc(curTimenode) {
    ...
    if (targetLocation >= this.data.scrollThreshold) {
      for (let i = this.data.scrollThreshold; i < targetLocation; i++) {
        moveDistance += this.lrcstrollschema.domArr[i].height + 10; //每次移动每行高度+边框高度
      }
      this.setData({
  ...
  lrcscroll(isScroll) {
    ...
    //开启定时器
    if (isScroll) {
      if (!this.lrcscroll.isFirstEntry) { //若是第一次进入歌词页面(需要等到到达第一句歌词的时间)
  ...
  async lrcstrollschema(delay, nextLocation) {
    let query = wx.createSelectorQuery(); //获取歌词dom节点
    query.selectAll('.songlrc').boundingClientRect((res) => {
      this.lrcstrollschema.domArr = res
    }).exec();
    if (typeof (this.lrcstrollschema.domArr) != undefined) {
      this.data.scrollTimer = setTimeout(() => { //开启滚动定时器
        let currentDistance = nextLocation <= this.data.scrollThreshold ? 0 : this.data.scrollTop +
          this.lrcstrollschema.domArr[this.data.lrclocation - this.data.scrollThreshold].height + 10; //计算每行的滚动距离
        this.setData({
        ...
        if (this.data.lrclocation < this.data.lrctime.length - 1) { //判断是否到最后
          this.lrcstrollschema((this.data.lrctime[this.data.lrclocation + 1] -
            this.data.lrctime[this.data.lrclocation]), this.data.lrclocation + 1); //通过时间点之差计算滚动时延（参数单位为毫秒）
        }
      }, delay);
    }
  },
  ...
  async collectsong() {
    if (this.data.iscollect) { //取消收藏
      ...
      let cancelcollect = await request.request2('/delcollect', {
      ...
      let collect = await request.request2('/addcollect', {
        songid: this.data.songid,
        username: wx.getStorageSync('username'),
        singer: this.data.songinfo.ar[0].name,
        songname: this.data.songinfo.name,
      }, 'POST');
      // console.log(collect);
    }
  },
  ...
  async judgeCollect() {
    ...
    let heartsongs = collection.data.data; //存放心动歌曲
    for (let i = 0; i < heartsongs.length; i++) {
      if (heartsongs[i].songid == this.data.songid) {
        this.setData({
          iscollect: true,
        })
        break;
  ...
  //后台切入当前歌曲时，歌词滚动到当前所唱位置
  async repeatscroll() {
    let query = wx.createSelectorQuery(); //获取歌词dom节点
    query.selectAll('.songlrc').boundingClientRect((res) => {
      let domArr = res;
      //  console.log(domArr);
      let curtime_ = this.backgroundAudioManager.currentTime * 1000;
      let curindex = tool.nearlyindex(this.data.lrctime, curtime_); //当前所唱歌词索引
      // console.log(curindex);
      let moveDistance = 0;
      if (curindex >= this.data.scrollThreshold) {
        for (let i = this.data.scrollThreshold; i < curindex; i++) {
          moveDistance += domArr[i].height + 10; //每次移动每行高度+边框高度
        }
        // console.log(moveDistance);
        this.setData({
          lrclocation: curindex,
          scrollTop: moveDistance,
        })
        this.lrcscroll(this.data.isPlay); //滑动到指定位置后歌词继续滚动(如果不再次调用该方法会回到点击前位置滚动，有bug)
      }
  ...
  //歌曲存入历史记录
  addhistory() {
    let historysong = wx.getStorageSync('historysong');
    let time = new Date();
    let thistime = moment(time).format("YYYY-MM-DD HH:mm");
    ...
    wx.setStorageSync('historysong', historysong);
  },

  //判断当前歌曲在歌单里位置
  findsongindex() {
    for (let i = 0; i < this.data.songs.length; i++) {
      if (this.data.songid == this.data.songs[i].id) {
        this.setData({
          cursongindex: i,
        })
        break;
